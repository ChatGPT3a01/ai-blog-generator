<template>
  <div class="container" style="max-width: 1200px;">
    ...
    <div v-else-if="records.length === 0" class="empty-state-large">
      ...
      <h3>暫無相關記錄</h3>
      <p class="empty-tips">去建立一個新的作品吧</p>
    </div>
...
async function regenerateHistoryImage(index: number) {
  ...
  try {
    ...
    if (result.success && result.image_url) {
      ...
      regeneratingImages.value.delete(index)
      alert('重新生成失敗: ' + (result.error || '未知錯誤'))
    }
  } catch (e) {
    regeneratingImages.value.delete(index)
    alert('重新生成失敗: ' + String(e))
  }
}
...
async function handleScanAll() {
  ...
  try {
    ...
    if (result.success) {
      let message = `掃描完成！\n`
      message += `- 總任務數: ${result.total_tasks || 0}\n`
      message += `- 同步成功: ${result.synced || 0}\n`
      message += `- 同步失敗: ${result.failed || 0}\n`

      if (result.orphan_tasks && result.orphan_tasks.length > 0) {
        message += `- 孤立任務（無記錄）: ${result.orphan_tasks.length} 個\n`
      }
