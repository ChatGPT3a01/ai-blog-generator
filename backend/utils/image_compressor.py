"""圖片壓縮工具"""
import io
from PIL import Image
from typing import Optional


def compress_image(
    image_data: bytes,
    max_size_kb: int = 200,  # 預設200KB
    quality_start: int = 85,
    quality_min: int = 20,
    max_dimension: int = 2048
) -> bytes:
    """
    壓縮圖片到指定大小以內

    Args:
        image_data: 原始圖片資料
        max_size_kb: 最大檔案大小（KB）
        quality_start: 起始壓縮品質（1-100）
        quality_min: 最低壓縮品質（1-100）
        max_dimension: 最大邊長（像素）

    Returns:
        壓縮後的圖片資料
    """
    max_size_bytes = max_size_kb * 1024

    # 如果原圖已經小於目標大小，直接返回
    if len(image_data) <= max_size_bytes:
        return image_data

    try:
        # 開啟圖片
        img = Image.open(io.BytesIO(image_data))

        # 轉換為 RGB（處理 RGBA 等格式）
        if img.mode in ('RGBA', 'LA', 'P'):
            background = Image.new('RGB', img.size, (255, 255, 255))
            if img.mode == 'P':
                img = img.convert('RGBA')
            background.paste(img, mask=img.split()[-1] if img.mode in ('RGBA', 'LA') else None)
            img = background
        elif img.mode != 'RGB':
            img = img.convert('RGB')

        # 如果圖片尺寸過大，先縮小
        width, height = img.size
        if width > max_dimension or height > max_dimension:
            ratio = min(max_dimension / width, max_dimension / height)
            new_width = int(width * ratio)
            new_height = int(height * ratio)
            img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)

        # 逐步降低品質直到滿足大小要求
        quality = quality_start
        compressed_data = None

        while quality >= quality_min:
            output = io.BytesIO()
            img.save(output, format='JPEG', quality=quality, optimize=True)
            compressed_data = output.getvalue()

            if len(compressed_data) <= max_size_bytes:
                break

            quality -= 5

        # 如果還是太大，進一步縮小尺寸
        if len(compressed_data) > max_size_bytes:
            width, height = img.size
            while len(compressed_data) > max_size_bytes and max(width, height) > 512:
                width = int(width * 0.9)
                height = int(height * 0.9)
                img_resized = img.resize((width, height), Image.Resampling.LANCZOS)

                output = io.BytesIO()
                img_resized.save(output, format='JPEG', quality=quality_min, optimize=True)
                compressed_data = output.getvalue()

        original_size_kb = len(image_data) / 1024
        compressed_size_kb = len(compressed_data) / 1024
        compression_ratio = (1 - compressed_size_kb / original_size_kb) * 100

        print(f"[Compress] {original_size_kb:.1f}KB -> {compressed_size_kb:.1f}KB ({compression_ratio:.1f}%)")

        return compressed_data

    except Exception as e:
        print(f"[Compress] Failed, return original: {e}")
        return image_data


def compress_images(images: list[bytes], max_size_kb: int = 200) -> list[bytes]:
    """
    批次壓縮圖片

    Args:
        images: 圖片資料列表
        max_size_kb: 最大檔案大小（KB）

    Returns:
        壓縮後的圖片資料列表
    """
    return [compress_image(img, max_size_kb) for img in images]
