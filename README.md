...
### 文字生成
- 8 種作家筆風：專業、輕鬆、幽默、詩意、故事、教學、新聞、行銷

### 圖片生成
- 8 種圖片風格：扁平插畫、3D 卡通、水彩、寫實、日系動漫、極簡、復古、科技
- 支援上傳參考圖片（保持風格一致）
- **智能 Thinking Chain**：啟用 AI 思考鏈，提升圖片生成品質與穩定性
- **Unsplash 備用圖庫**：AI 生成失敗時，可從免費圖庫選擇替代圖片
...
#### 選項 1：Google Gemini 生圖（推薦，免費額度多）

使用與文字相同的 Google Gemini API Key：
1. 系統設定 → 圖片生成 → 新增
2. 類型：選擇「Google GenAI」
3. API Key：貼上 Gemini Key
4. 模型：選擇 `gemini-3-pro-image-preview`（推薦）
5. 測試連線 → 儲存 → 啟用

> 已內建 Thinking Chain 優化，可生成高品質圖片
